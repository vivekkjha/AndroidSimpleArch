...
/**
 * Wrapper of recycler view with frequently used methods wrapped in one class, pagination also included
 */
public abstract class AppRecycler<T> extends RecyclerView {
    ...
    private ICancelableTask mCancelableTask;
    private Pagination<T> mPagination;
    private IProgressWatcher mPaginatedProgressWatcher = null;
    ...
    private void init()
    {
        this.mRecyclerAdapter = new RecyclerView.Adapter()
        {
            @Override
            ...
            public void onBindViewHolder(ViewHolder holder,final int position) {
                ...
                if (position == getLoadNextPosition()) {
                    loadPaginatedViews();
                }
            }
    ...
    /**
     * Load pagination views
     */
    private void loadPaginatedViews()
    {
        if(mCancelableTask!=null || noMoreResult) return;
        if(getPagination() == null) return;
        mCancelableTask = Invoker.run(new BGTask<Void, List<T>>() {
            @Override
            public List<T> performOperation(Void... voids) throws Exception {
                return mPagination.loadNext(getNextPageIndex(),getPageSize());
            }

            @Override
            public void updateUI(List<T> ts) {
                mCancelableTask = null;
                if(ts!=null && ts.size()!=0) {
                    if (!appendItemsOnPagination())
                        mItems.clear();
                    addItems(ts);
                }
                else
            ...
            public boolean handleErrorInUI(Exception t) {
                mCancelableTask = null;
                mPagination.onError(t);
                return true;
            }
        },getPaginatedProgressWatcher());
    ...
    public void addItem(T item) {
        this.addItem(this.mItems.size() - 1, item);
    }
    ...
    public void setPagination(Pagination<T> mPagination) {
        this.mPagination = mPagination;
    }

    public IProgressWatcher getPaginatedProgressWatcher() {
        return mPaginatedProgressWatcher;
    }

    public void setPaginatedProgressWatcher(IProgressWatcher mPaginatedProgressWatcher) {
        this.mPaginatedProgressWatcher = mPaginatedProgressWatcher;
    }
